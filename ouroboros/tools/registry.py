...
CORE_TOOL_NAMES = {
    ...
    "run_shell", "run_powershell", "claude_code_edit",
    ...
    "desktop_screenshot", "desktop_click", "desktop_move", "desktop_scroll",
    "desktop_drag", "desktop_type", "desktop_press_key",
...
class ToolRegistry:
    ...
    def execute(self, name: str, args: Dict[str, Any]) -> str:
        entry = self._entries.get(name)
        if entry is None:
            return f"WARNING: Unknown tool: {name}. Available: {', '.join(sorted(self._entries.keys()))}"
        try:
            return entry.handler(self._ctx, **args)
        except TypeError as e:
            return f"WARNING: TOOL_ARG_ERROR ({name}): {e}"
        except Exception as e:
            return f"WARNING: TOOL_ERROR ({name}): {e}"

    ...
    try:
        ...
        if metrics.get("largest_files"):
            ...
            for path, size in metrics["largest_files"][:10]:
                marker = " WARNING: OVERSIZED" if size > 1000 else ""
        ...
        if metrics.get("longest_functions"):
            ...
            for path, start, length in metrics["longest_functions"][:10]:
                marker = " WARNING: OVERSIZED" if length > 150 else ""
        ...
        if oversized_funcs or oversized_mods:
            lines.append("\n### WARNING: Bible Violations (Principle 5: Minimalism)")
            if oversized_funcs:
            ...
            lines.append("\nOK: No Bible violations detected (all functions < 150 lines, all modules < 1000 lines)")

        return "\n".join(lines)

    except Exception as e:
        log.warning("codebase_health failed: %s", e, exc_info=True)
        return f"WARNING: Failed to compute codebase health: {e}"

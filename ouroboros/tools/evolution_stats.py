  ...
  try {
    ...
    if (stats) {
      ...
      stats.innerHTML = [
        ...
        ['[BG]', 'Self Growth',     `${(first.system_bytes/1024).toFixed(1)} -> ${(last.system_bytes/1024).toFixed(1)} KB`,  `Generated ${data.generated_at ? new Date(data.generated_at).toLocaleDateString() : ''}`],
    ...
    if (loading) { loading.style.display = 'block'; loading.textContent = `WARNING: ${e.message}`; }

...
def _gh_cmd(args: List[str], ctx: ToolContext, timeout: int = 30, input_data: Optional[str] = None) -> str:
    ...
    try:
        ...
        if res.returncode != 0:
            ...
            return f"WARNING: GH_ERROR: {err.split(chr(10))[0][:200]}"
        return res.stdout.strip()
    except FileNotFoundError:
        return "WARNING: GH_ERROR: `gh` CLI not found."
    except subprocess.TimeoutExpired:
        return f"WARNING: GH_TIMEOUT: exceeded {timeout}s."
    except Exception as e:
        return f"WARNING: GH_ERROR: {e}"
...
def _list_issues(ctx: ToolContext, state: str = "open", labels: str = "", limit: int = 20) -> str:
    ...
    try:
        ...
        return f"WARNING: Failed to parse issues JSON: {raw[:500]}"
...
def _get_issue(ctx: ToolContext, number: int) -> str:
    ...
    if number <= 0:
        return "WARNING: issue number must be positive"
    ...
    try:
        ...
        return f"WARNING: Failed to parse issue JSON: {raw[:500]}"
...
def _comment_on_issue(ctx: ToolContext, number: int, body: str) -> str:
    ...
    if not body or not body.strip():
        return "WARNING: Comment body cannot be empty."
    ...
    if raw.startswith("WARNING:"):
        return raw
    return f"OK: Comment added to issue #{number}."

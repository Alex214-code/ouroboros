...
def _run_pre_push_tests(ctx: ToolContext) -> Optional[str]:
    ...
    try:
        ...
        return "WARNING: PRE_PUSH_TEST_ERROR: pytest timed out after 30 seconds"

    except FileNotFoundError:
        return "WARNING: PRE_PUSH_TEST_ERROR: pytest not installed or not found in PATH"

    except Exception as e:
        log.warning(f"Pre-push tests failed with exception: {e}", exc_info=True)
        return f"WARNING: PRE_PUSH_TEST_ERROR: Unexpected error running tests: {e}"
...
def _git_push_with_tests(ctx: ToolContext) -> Optional[str]:
    ...
    if test_error:
        ...
        return f"WARNING: PRE_PUSH_TESTS_FAILED: Tests failed, push blocked.\n{test_error}\nCommitted locally but NOT pushed. Fix tests and push manually."
    ...
    try:
        ...
        return f"WARNING: GIT_ERROR (push): {e}\nCommitted locally but NOT pushed."
...
def _repo_write_commit(ctx: ToolContext, path: str, content: str, commit_message: str) -> str:
    ...
    if not commit_message.strip():
        return "WARNING: ERROR: commit_message must be non-empty."
    ...
    try:
        ...
        try:
            ...
            return f"WARNING: FILE_WRITE_ERROR: {e}"
        try:
            run_cmd(["git", "add", safe_relpath(path)], cwd=ctx.repo_dir)
        except Exception as e:
            return f"WARNING: GIT_ERROR (add): {e}"
        try:
            run_cmd(["git", "commit", "-m", commit_message], cwd=ctx.repo_dir)
        except Exception as e:
            return f"WARNING: GIT_ERROR (commit): {e}"
...
def _repo_commit_push(ctx: ToolContext, commit_message: str, paths: Optional[List[str]] = None) -> str:
    ...
    try:
        ...
        try:
            ...
            return f"WARNING: GIT_ERROR (status): {e}"
        if not status.strip():
            return "WARNING: GIT_NO_CHANGES: nothing to commit."
        try:
            run_cmd(["git", "commit", "-m", commit_message], cwd=ctx.repo_dir)
        except Exception as e:
            return f"WARNING: GIT_ERROR (commit): {e}"
    ...
    if paths is not None:
        try:
            ...
            if untracked.strip():
                ...
                result += f"\nWARNING: untracked files remain: {files} — they are NOT in git. Use repo_commit_push without paths to add everything."
        except Exception:

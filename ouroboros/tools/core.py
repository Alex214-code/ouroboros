...
def _list_dir(root: pathlib.Path, rel: str, max_entries: int = 500) -> List[str]:
    ...
    if not target.exists():
        return [f"WARNING: Directory not found: {rel}"]
    if not target.is_dir():
        return [f"WARNING: Not a directory: {rel}"]
    items = []
    try:
        ...
        items.append(f"WARNING: Error listing: {e}")
    return items
...
def _send_photo(ctx: ToolContext, image_base64: str, caption: str = "") -> str:
    ...
    if not ctx.current_chat_id:
        return "WARNING: No active chat — cannot send photo."
    ...
    if image_base64 == "__last_screenshot__":
        if not ctx.browser_state.last_screenshot_b64:
            return "WARNING: No screenshot stored. Take one first with browse_page(output='screenshot')."
        actual_b64 = ctx.browser_state.last_screenshot_b64

    if not actual_b64 or len(actual_b64) < 100:
        return "WARNING: image_base64 is empty or too short. Take a screenshot first with browse_page(output='screenshot')."
...
def _summarize_dialogue(ctx: ToolContext, last_n: int = 200) -> str:
    ...
    if not chat_path.exists():
        return "WARNING: chat.jsonl not found"
    ...
    try:
        ...
        if not entries:
            return "WARNING: No chat entries found"
...
Now write a comprehensive summary:"""
        ...
        if not summary:
            return "WARNING: LLM returned empty summary"
...
def get_tools() -> List[ToolEntry]:
    return [
        ...
        ToolEntry("drive_read", {
            "name": "drive_read",
            "description": "Read a UTF-8 text file from state directory (memory, logs, etc).",
            "parameters": {"type": "object", "properties": {"path": {"type": "string"}}, "required": ["path"]},
        }, _drive_read),
        ...
        ToolEntry("drive_write", {
            "name": "drive_write",
            "description": "Write a UTF-8 text file to state directory.",
            "parameters": {"type": "object", "properties": {
                "path": {"type": "string"},
                "content": {"type": "string"},

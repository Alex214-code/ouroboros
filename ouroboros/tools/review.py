...
def _handle_multi_model_review(ctx: ToolContext, content: str = "", prompt: str = "", models: list = None) -> str:
    ...
    # Local backend (Ollama): single-model review via LLMClient
    if os.environ.get("OUROBOROS_LLM_BACKEND") == "ollama":
        return _handle_local_review(ctx, content, prompt)
...
def _handle_local_review(ctx: ToolContext, content: str, prompt: str) -> str:
    """Single-pass review using local Ollama model. Replaces multi-model for local mode."""
    ...
    try:
        ...
        msg, usage = llm.chat(messages=messages, model=model, reasoning_effort="medium", max_tokens=2048)
        ...
        log.error("Local review failed: %s", e, exc_info=True)
        return json.dumps({"error": f"Local review failed: {e}"}, ensure_ascii=False)

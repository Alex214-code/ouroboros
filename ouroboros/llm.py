...
def fetch_openrouter_pricing() -> Dict[str, Tuple[float, float, float]]:
    ...
    try:
        ...
        prefixes = ("anthropic/", "openai/", "google/", "meta-llama/", "x-ai/", "qwen/", "stepfun/", "deepseek/")
...
class LLMClient:
    """LLM API wrapper. Supports OpenRouter and Ollama backends."""
    ...
    def __init__(
        ...
        if self._backend == "ollama":
            self._base_url = base_url or os.environ.get("OLLAMA_BASE_URL", "http://localhost:11434/v1")
            self._api_key = "ollama"  # Ollama не требует ключа
        else:
            self._base_url = base_url or "https://openrouter.ai/api/v1"
            self._api_key = api_key or os.environ.get("OPENROUTER_API_KEY", "")
        self._client = None
        log.info(f"LLMClient initialized: backend={self._backend}, base_url={self._base_url}")

    @property
    def is_local(self) -> bool:
        """True if using a local backend (Ollama)."""
        return self._backend == "ollama"
    ...
    def chat(
        ...
        if self._backend == "ollama":
            # Ollama: minimal kwargs, no extra_body

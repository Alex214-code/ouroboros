...
def _build_runtime_section(env: Any, task: Dict[str, Any]) -> str:
    ...
    # --- System info ---
    system_info = {"os": os.name, "platform": "windows" if os.name == "nt" else "linux"}
    if os.name == "nt":
        system_info["home"] = str(pathlib.Path.home())
        system_info["shell_tools"] = ["run_shell (cmd/exe)", "run_powershell", "fs_read", "fs_write", "fs_list"]
    else:
        system_info["shell_tools"] = ["run_shell (bash)", "fs_read", "fs_write", "fs_list"]
...
def _compact_tool_result(msg: dict, content: str) -> dict:
    ...
    """
    is_error = content.startswith("WARNING:")

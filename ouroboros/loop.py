...
def _execute_single_tool(
    ...
    try:
        ...
        result = f"WARNING: TOOL_ARG_ERROR: Could not parse arguments for '{fn_name}': {e}"
        return {
    ...
    try:
        ...
        tool_ok = False
        result = f"WARNING: TOOL_ERROR ({fn_name}): {type(e).__name__}: {e}"
    ...
    is_error = (not tool_ok) or str(result).startswith("WARNING:")
...
def _make_timeout_result(
    ...
    result = (
        f"WARNING: TOOL_TIMEOUT ({fn_name}): exceeded {timeout_sec}s limit. "
...
def _maybe_inject_self_check(
    ...
    reminder = (
        ...
        f"[CHART] Context: ~{ctx_tokens} tokens | Cost so far: ${task_cost:.2f} | "
    ...
    emit_progress(f"[SYNC] Checkpoint {checkpoint_num} at round {round_idx}: ~{ctx_tokens} tokens, ${task_cost:.2f} spent")
...
def _setup_dynamic_tools(tools_registry, tool_schemas, messages):
    ...
    def _handle_enable_tools(ctx=None, tools: str = "", **kwargs):
        ...
        parts = []
        if enabled:
            parts.append(f"OK: Enabled: {', '.join(enabled)}")
        if not_found:
            parts.append(f"FAIL: Not found: {', '.join(not_found)}")
    ...
    try:
        while True:
            ...
            if round_idx > MAX_ROUNDS:
                finish_reason = f"WARNING: Task exceeded MAX_ROUNDS ({MAX_ROUNDS}). Consider decomposing into subtasks via schedule_task."
            ...
            if msg is None:
                ...
                if fallback_model is None:
                    return (
                        f"WARNING: Failed to get a response from model {active_model} after {max_retries} attempts. "
                ...
                if msg is None:
                    return (
                        f"WARNING: Failed to get a response from the model after {max_retries} attempts. "

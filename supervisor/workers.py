...
def handle_chat_direct(chat_id: int, text: str, image_data: Optional[Union[Tuple[str, str], Tuple[str, str, str]]] = None) -> None:
    try:
        ...
        import traceback
        err_msg = f"WARNING: Error: {type(e).__name__}: {e}"
        append_jsonl(
...
def _verify_worker_sha_after_spawn(events_offset: int, timeout_sec: float = 90.0) -> None:
    ...
    if not ok and st.get("owner_chat_id"):
        send_with_budget(
            ...
            f"WARNING: Worker SHA mismatch after spawn: expected {expected_sha[:8]}, got {(observed_sha or 'unknown')[:8]}",
        )
            ...
            if w.busy_task_id is None and PENDING:
                ...
                if task_type in ("evolution", "review"):
                    ...
                    if st.get("owner_chat_id"):
                        emoji = '[EVO]' if task_type == 'evolution' else '[!]'
                        send_with_budget(
                            int(st["owner_chat_id"]),
...
def ensure_workers_healthy() -> None:
    ...
    if len(CRASH_TS) >= 3:
        ...
        if st.get("owner_chat_id"):
            send_with_budget(
                int(st["owner_chat_id"]),
                "WARNING: Frequent worker crashes. Multiprocessing workers disabled, "

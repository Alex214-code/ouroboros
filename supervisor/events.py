...
def _handle_restart_request(evt: Dict[str, Any], ctx: Any) -> None:
    ...
    if st.get("owner_chat_id"):
        ctx.send_with_budget(
            ...
            f"RESTART: Restart requested by agent: {evt.get('reason')}",
        )
    ...
    if not ok:
        if st.get("owner_chat_id"):
            ctx.send_with_budget(int(st["owner_chat_id"]), f"WARNING: Restart skipped: {msg}")
        return
    ctx.kill_workers()
...
def _handle_promote_to_stable(evt: Dict[str, Any], ctx: Any) -> None:
    ...
    try:
        ...
        if st.get("owner_chat_id"):
            ctx.send_with_budget(
                int(st["owner_chat_id"]),
                f"OK: Promoted: {ctx.BRANCH_DEV} -> {ctx.BRANCH_STABLE} ({new_sha[:8]})",
            )
    except Exception as e:
        st = ctx.load_state()
        if st.get("owner_chat_id"):
            ctx.send_with_budget(
                int(st["owner_chat_id"]),
                f"FAIL: Failed to promote to stable: {e}",
            )
...
def _handle_schedule_task(evt: Dict[str, Any], ctx: Any) -> None:
    ...
    if depth > 3:
        ...
        if owner_chat_id:
            ctx.send_with_budget(int(owner_chat_id), f"WARNING: Task rejected: subtask depth limit (3) exceeded")
        return
    ...
    if owner_chat_id and desc:
        ...
        if dup_id:
            ...
            ctx.send_with_budget(int(owner_chat_id), f"WARNING: Task rejected: semantically similar to already active task {dup_id}")
            return
...
def _handle_cancel_task(evt: Dict[str, Any], ctx: Any) -> None:
    ...
    if owner_chat_id:
        ctx.send_with_budget(
            int(owner_chat_id),
            f"{'OK:' if ok else 'FAIL:'} cancel {task_id or '?'} (event)",
        )
...
def _handle_toggle_evolution(evt: Dict[str, Any], ctx: Any) -> None:
    ...
    if st.get("owner_chat_id"):
        ...
        ctx.send_with_budget(int(st["owner_chat_id"]), f"[EVO] Evolution: {state_str} (via agent tool)")
...
def _handle_toggle_consciousness(evt: Dict[str, Any], ctx: Any) -> None:
    ...
    st = ctx.load_state()
    if st.get("owner_chat_id"):
        ctx.send_with_budget(int(st["owner_chat_id"]), f"[BG] {result}")

    ...
    for task_id, meta in list(RUNNING.items()):
        ...
        if runtime_sec >= SOFT_TIMEOUT_SEC and not bool(meta.get("soft_sent")):
            ...
            if owner_chat_id:
                send_with_budget(
                    owner_chat_id,
                    f"[TIMER] Task {task_id} running for {int(runtime_sec)}s. "
        ...
        if owner_chat_id:
            if requeued:
                send_with_budget(owner_chat_id, (
                    f"PANIC: Hard-timeout: task {task_id} killed after {int(runtime_sec)}s.\n"
...
def enqueue_evolution_task_if_needed() -> None:
    ...
    if consecutive_failures >= 3:
        ...
        send_with_budget(
            int(owner_chat_id),
            f"[EVO]WARNING: Evolution paused: {consecutive_failures} consecutive failures. "
    ...
    save_state(st)
    send_with_budget(int(owner_chat_id), f"[EVO] Evolution #{cycle}: {tid}")

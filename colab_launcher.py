        ...
        try:
            ...
            if idle_sec >= HARD_TIMEOUT_SEC:
                ...
                if st.get("owner_chat_id"):
                    send_with_budget(
                        ...
                        f"WARNING: Task stuck ({int(total_sec)}s without progress). "
                        f"Restarting agent.",
                    )
                reset_chat_agent()
                soft_warned = False
            ...
            if idle_sec >= SOFT_TIMEOUT_SEC and not soft_warned:
                ...
                if st.get("owner_chat_id"):
                    send_with_budget(
                        int(st["owner_chat_id"]),
                        f"[TIMER] Task running for {int(total_sec)}s, "
...
def _handle_supervisor_command(text: str, chat_id: int, tg_offset: int = 0):
    ...
    if lowered.startswith("/restart"):
        ...
        if not ok:
            send_with_budget(chat_id, f"WARNING: Restart cancelled: {msg}")
            return True
        kill_workers()
    ...
    if lowered.startswith("/evolve"):
        ...
        send_with_budget(chat_id, f"[EVO] Evolution: {state_str}")
    ...
    if lowered.startswith("/bg"):
        ...
        if action in ("start", "on", "1"):
            ...
            send_with_budget(chat_id, f"[BG] Background consciousness: {bg_status}")
...
try:
    ...
    log.info("[BG] Background consciousness auto-started (default: always on)")
except Exception as e:
    ...
    for upd in updates:
        ...
        if agent._busy:
            ...
            if image_data:
                ...
                send_with_budget(chat_id, "[ATTACH] Photo received, but a task is in progress. Send again when I'm free.")
            elif text:
                agent.inject_message(text)
